...
class TokenTransfer extends React.Component {
  ...
  onApprove = async (event) => {
    ...
    if (window.ethereum) {
      const { recepientAccountAddress, amount } = this.state;
      ...
      try {
        ...
        await this.props.ERC20Token.methods
          .approve(recepientAccountAddress, calculatedTransferValue)
          .send({
        ...
        var paragraph =
          '<p id="transactionSuccessMessage" style="">Approve <span class="amount">' +
          amount +
          '</span> Token to Address : <span class="amount">' +
          recepientAccountAddress +
          "</span> Success</p>";
        var aTag =
          '<a target="_blank" tyle="" id="transactionDetailsLink" href="https://rinkeby.etherscan.io/tx/' +
          TransactionHash +
          '">';
        aTag +=
    ...
    return (
      <Layout>
        <div id="transferPageWrapper">
          ...
          <h3>Approve Token</h3>
          <Form onSubmit={this.onApprove} error={!!this.state.errorMessage}>
            <Form.Field>
              <label>Spender Account Address</label>
              <Input
                label="Address"
                labelPosition="right"
