...
class TokenTransfer extends React.Component {
  ...
  static async getInitialProps(props) {
    ...
    /*
		...
		*/
    const connectedAccountAddress = await window.ethereum.request({
      method: "eth_requestAccounts",
    });
    const connectedAccountBalance = await ERC20Token.methods
      .balanceOf(connectedAccountAddress[0])
      .call();
  ...
  onAllowance = async (event) => {
    ...
    if (window.ethereum) {
      const { tokenOwner, spender } = this.state;
  ...
  onTransfer = async (event) => {
    ...
    if (window.ethereum) {
      const { senderAccountAddress, recepientAccountAddress, amount } =
        this.state;
      ...
      try {
        ...
        /*
				...
				*/
        const connectedAccountNewBalance = await this.props.ERC20Token.methods
          .balanceOf(this.props.connectedAccountAddress[0])
          .call();
        document.getElementById("tokenAmount").innerHTML =
          connectedAccountNewBalance;
        ...
        var paragraph =
          '<p id="transactionSuccessMessage" style="">Transfer <span class="amount">' +
          amount +
          '</span> Token to Address : <span class="amount">' +
          recepientAccountAddress +
          "</span> Success</p>";
        var aTag =
          '<a target="_blank" style="" id="transactionDetailsLink" href="https://rinkeby.etherscan.io/tx/' +
          TransactionHash +
          '">';
        aTag +=
          'Transaction Details : <span id="transactionHash">' +
          TransactionHash +
          "</span></a>";
        var wrapperClose = "</div>";
        document.getElementById("transferDetails").innerHTML =
          wrapperOpen + paragraph + aTag + wrapperClose;
      ...
      <Layout>
        <div id="transferPageWrapper">
          <div id="account-info">
            ...
            <h4>
              <span id="tokenAmount">{this.props.connectedAccountBalance}</span>{" "}
              {this.props.tokenSymbol}
            </h4>
          </div>
          <h3>Token Allowance</h3>
          <Form
            onSubmit={this.onAllowance}
            error={!!this.state.errorMessageAllowance}>
            <Form.Field>
            ...
            <Message
              error
              header="Oops!"
              content={this.state.errorMessageAllowance}
            />
            <Button loading={this.state.loadingAllowance} primary>
              Allowance
            </Button>
          ...
          <Form
            ...
            error={!!this.state.errorMessageTransfer}>
            <Form.Field>
              ...
              <Input
                ...
                onChange={(event) =>
                  this.setState({ senderAccountAddress: event.target.value })
                }
              />
              ...
              <Input
                ...
                onChange={(event) =>
                  this.setState({ recepientAccountAddress: event.target.value })
                }
              />
              <label>Amount</label>
              <Input
            ...
            />
            <Button loading={this.state.loadingTransfer} primary>
              Transfer
            </Button>
